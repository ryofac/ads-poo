...
import { obterTexto, showCenterText, enterToContinue } from './utils';
...
async function main() {
    ...
    const postagens: Postagem[] = MICROBLOG.postagens
    ...
    do {
        ...
        if (opcao == 2) {
            ...
            if (id != -1) {
                ...
                showCenterText("Esse Id não existe não oh doido")
            }
        }
        if (opcao == 3) {
    ...
    function showPostagens() {
        for (let i: number = 0; i < postagens.length; i++) {
            showCenterText(`ID: ${i} - ${postagens[i]} \n`)
        }
